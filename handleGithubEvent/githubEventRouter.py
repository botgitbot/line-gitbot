...
def githubEventRouter(payload, group_id):
    ...
    if("zen" in payload.keys()):
        ...
        if not isWebhookRecorded(group_id,payload["sender"]["login"], payload["repository"]["name"]):
            addWebhookToDatabase(group_id, payload["sender"]["login"], payload["repository"]["name"])
